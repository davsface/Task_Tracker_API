        ...
        for attribute in content:
            if attribute in ["id", "name", "type", "status", "assigned to"]:
                ...
                if attribute == "assigned to":
                    return {"Error": "Modifying equipment assignment is not allowed, please use task/<id>/equipment endpoint"}, 403
...
def equipment_read_delete(id):
    ...
    if request.method == 'DELETE':
        ...
        # get tasks and loop through their equipment until id is found and remove from task
        query = client.query(kind=constants.task)
        tasks = query.fetch()
        updated_tasks = []
        for task in tasks:
            for equipment in task['equipment']:
                if equipment['id'] == int(id):
                    task['equipment'].remove(equipment)
